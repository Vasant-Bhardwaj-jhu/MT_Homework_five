...
class EncoderRNN(nn.Module):
    ...
    def forward(self, encoded_sentence : torch.Tensor):
        ...
        out_hidden_hs = torch.zeros(len_sentence, 2 * self.hidden_size, device=device)
        ...
        for i in range(len_sentence):
            f_hidden_h, f_hidden_c = self.forward_LSTM(f_hidden_h, f_hidden_c, word_embeddings[i])
            forward_out[i] = f_hidden_h
        ...
        for i in reversed(range(len_sentence)):
            b_hidden_h, b_hidden_c = self.backward_LSTM(b_hidden_h, b_hidden_c, word_embeddings[i])
            backward_out[i] = b_hidden_h
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, prev_word, hidden_s, hidden_c, encoder_outputs, attn_contxts):
        ...
        # Because currently 1d and MaxPool requires 2d, we arbitrarily expand context
        # This is not necessary if input is guranteed multi-dimensional
        # nn.Sequential cannot handle multiple positional arguments, so...
        # *other x len_dict
        word_probs = self.out(self.out_first(hidden_s, input_encoded, context.unsqueeze(0)))
    ...
    def train_sentence(self, encoder_outputs : torch.Tensor, target_sentence : torch.Tensor, criterion):
        ...
        """
        if self.model_select & ModelOptions.TBPTT:
            return self.train_sentence_trunc(encoder_outputs, target_sentence, criterion)
    ...
    def decode_sentence_beam(self, encoder_outputs : torch.Tensor):
        ...
        while iteration < self.max_length and len(final_candidate_sequences) <= self.search_size:
            ...
            for neg_log_prob, decoded_words, attn_weights_per_word, hidden_s, hidden_c in candidate_sequences:
                ...
                new_attn_weights_per_word = attn_weights_per_word + [attn_weights.detach().squeeze(-1)] # In place possible?
