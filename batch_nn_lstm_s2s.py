...
class EncoderRNN(nn.Module):
    ...
    def encode_sentence(self, encoded_sentence : torch.Tensor):
        return self(encoded_sentence.unsqueeze(0))
...
class AttnDecoderRNN(nn.Module):
    ...
    def get_initial_states(self, encoder_outputs, device) -> Tuple[torch.Tensor, torch.Tensor]:
        # 1 x hidden_size
        return self.init_hidden(encoder_outputs[:,0,self.hidden_size:]).unsqueeze(0), torch.zeros(1, encoder_outputs.size(0), self.hidden_size, device=device)
    ...
    def forward(self, prev_word, hidden_s, hidden_c, encoder_outputs, attn_contxts):
        ...
        # ...Unfortunately, hidden_s is stuck in (1,N,H)...
        attn_weights = self.attn_post(self.attn_hidden(hidden_s.squeeze(0)).unsqueeze(-2) + attn_contxts + self.attn_bias)
    ...
    def train_sentence(self, encoder_outputs : torch.Tensor, target_sentence : torch.Tensor, criterion):
        ...
        prev_word = torch.tensor([SOS_index], device=device).expand(encoder_outputs.size(-3))
